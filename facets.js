class FacetFiltersForm extends HTMLElement {
  renderSwatches() {
    const colorInputs = this.querySelectorAll('input[name*="color_category"]');

    colorInputs.forEach((input) => {
      const label = this.querySelector(`label[for="${input.id}"]`);
      const colorValue = input.value.toLowerCase();
      ...
      // Select and hide all the SVG elements inside the label
      const svgElements = label.querySelectorAll("svg");
      svgElements.forEach((svg) => {
        svg.style.display = "none";
      });
      // Select all span elements inside the label
      const spanElements = label.querySelectorAll("span");
      // Hide the first span if it exists
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === "childList") {
          console.log("Swatches modified:", mutation);
        }
      });
    });
  ...
  static renderPage(searchParams, event, updateURLHash = true) {
    ...
    // FacetFiltersForm.renderActiveFacets(parsedHTML, event); // Pass the event object
  }
  ...
  static renderActiveFacets(html, event) {
    ...
    // Add the following lines to call the renderSwatches() method with a delay
    const facetFiltersFormElement =
      document.querySelector("facet-filters-form");
    if (facetFiltersFormElement) {
      setTimeout(() => {
        facetFiltersFormElement.renderSwatches();
      }, 0);
    }
  ...
  onActiveFilterClick(event) {
    ...
    FacetFiltersForm.renderPage(url, event); // Pass the event object
  }
}
...
styleElement.textContent = swatchStyles;
